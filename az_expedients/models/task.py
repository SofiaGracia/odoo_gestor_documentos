...
class task(models.Model):
	_name = 'az_expedients.task'
	_description = 'Tarea'
	_inherit = ['mail.thread', 'mail.activity.mixin']

	name = fields.Char(string = 'Nombre', required = True, tracking = 1)
	state_id = fields.Many2one('az_expedients.statetask', string = 'Estado tarea', tracking = 1, group_expand='_group_expand_states')
	...
	def _group_expand_states(self, states, domain, order):
		return self.env['az_expedients.statetask'].search([])
